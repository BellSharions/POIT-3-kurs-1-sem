...
#define MAIL1 TEXT("\\\\DESKTOP-MAJO816\\mailslot\\Box")
...
int main()
{
	...
	HANDLE hMS;
	double t1, t2;
	char message[50] = "Hello from Mailslots-Client ";
	DWORD bytes;

    int countMessage = 0;
    std::cout << std::endl << "Count of messages: ";
    std::cin >> countMessage;

    try
    {
        if ((hMS = CreateFile(MAIL1, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, NULL, NULL)) == INVALID_HANDLE_VALUE)
        ...
        if (!WriteFile(hMS, "end", sizeof("end"), &bytes, NULL))
            throw SetErrorMail("WriteFile: ", GetLastError());
