...
int main()
{
	...
	try
	{
        while (true)
        {
            if ((hMS = CreateMailslot(MAIL, 500, MAILSLOT_WAIT_FOREVER, NULL)) == INVALID_HANDLE_VALUE)
            ...
            while (true)
            {
                if (!ReadFile(hMS,
                    ...
                    throw SetErrorMail("ReadFileError", GetLastError());
